...
public:
    ...
    RBNode<int> *addNode2(RBNode<int>*& nodePtr, int key)
    {
        RBNode<int> *newnode = new RBNode<int>::RBNode<int>(key, NULL, NULL, RBNode<int>::RED, NULL);
        
        
        if (nodePtr == NULL)
        {
            nodePtr = new RBNode<int>(key,NULL,NULL,RBNode<int>::RED,NULL);
            return nodePtr;
        }
        else if ( (nodePtr->data() < key) && (nodePtr->right() == NULL) ){
            nodePtr->set_right( addNode2(nodePtr->right(), key) );
            nodePtr->right()->set_parent(nodePtr);
            fixTree(nodePtr->right());
            return NULL;
        }
        
        else if ( (key < nodePtr->data() ) && (nodePtr->left() == NULL) ){
            nodePtr->set_left( addNode2(nodePtr->left(), key) );
            nodePtr->left()->set_parent(nodePtr);
            fixTree(nodePtr->left());
            return NULL;
        }
        
        else if ( nodePtr->data() < key){
            addNode2(nodePtr->right(),key);
        }
        
        else {
            addNode2(nodePtr->left(),key);
        }
        return NULL;
    }
